...
exports.getPackageStorePath = (storeDir, pkg, pkgStoreDir) => {
  ...
  return path.join(storeDir, `_${(pkgStoreDir || pkg.name).replace(/\//g, '_')}@${pkg.version}@${(pkgStoreDir || pkg.name)}`);
};
...
exports.copyInstall = async (src, options) => {
  ...
  const targetdir = options.ungzipDir || exports.getPackageStorePath(options.storeDir, realPkg, options.pkgStoreDir || '');
